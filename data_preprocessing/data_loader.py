...
from data_preprocessing.datasets import SVHN_truncated, CIFAR_truncated, MNIST_truncated, ImageFolder_custom
...
def _data_transforms_svhn(datadir):
    CIFAR_MEAN = (0.4376821, 0.4437697, 0.47280442)
    CIFAR_STD = (0.19803012, 0.20101562, 0.19703614)
...
def _data_transforms_imagenet(datadir=None):
...
def _data_transforms_other(datadir=None):
    mean = [0.485, 0.456, 0.406]
    std = [0.229, 0.224, 0.225]
    crop_scale = 0.08
    jitter_param = 0.4
...
def load_data(datadir):
    ...
    train_ds = dl_obj(datadir, train=True, transform=train_transform)
...
# partition_size=500
def partition_data(datadir, partition, n_nets, alpha,partition_size):
    ...
    if partition == "homo":
        ...
        #        while min_size < 10:
        while min_size < partition_size // class_num:
        ...
        for j in range(n_nets):
           ...
           # net_dataidx_map[j] = idx_batch[j]
            net_dataidx_map[j] = idx_batch[j][:partition_size]
...
def load_partition_data(data_dir, partition_method, partition_alpha, client_number, batch_size, partition_size):
    class_num, net_dataidx_map, traindata_cls_counts = partition_data(data_dir, partition_method, client_number,
                                                                      partition_alpha,partition_size)
    ...
    logging.info("train number = " + str(len(train_data_global) * 5))
    logging.info("test number = " + str(len(train_data_global) * 5))
